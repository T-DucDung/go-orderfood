...
func QueryGetBSF(sid int, startTime, endTime int64) string {
	return "SELECT Sum(quantity) as total_count, food_id from orderfood where order_id IN (select id FROM `order` where order_status = 5 and store_id = " + strconv.Itoa(sid) + " and updated_date BETWEEN " + strconv.FormatInt(startTime, 10) + " and " + strconv.FormatInt(endTime, 10) + ") GROUP BY  food_id  ORDER  by total_count DESC "
}
